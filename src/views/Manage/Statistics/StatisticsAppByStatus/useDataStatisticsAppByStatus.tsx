...
import {
  getStatisticalAppByHospitalChart,
  getStatisticalAppByStatusChart,
...
export default function useDataChartAppByStatus() {
  ...
  async function getDataChart() {
    setLoading(true);
    try {
      const response = await getStatisticalAppByStatusChart();
      ...
      setDataAllocation((prev) => {
        return {
          ...
          detail: response.data.data.dataByStatus.map((item: any) => {
            const amount = BN(item.totalAppByStatus);
    ...
    for (let i = 0; i < length; i++) {
      ...
      if (i < 4) {
        arr.push({
          ...
          title: token.name,
          data: [{ tokenName: token.name, blance: token.amount }],
        });
